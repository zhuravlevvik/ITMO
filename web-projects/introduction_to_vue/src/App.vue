<template>
    <div id="app">
        ...
        <Footer :num-of-posts="numberOfPosts" :num-of-users="numberOfUsers"/>
    </div>
</template>
...
export default {
    ...
    computed: {
      numberOfPosts: function () { return Object.values(this.posts).length },
      numberOfUsers: function () { return Object.values(this.users).length },
    },
    beforeCreate() {
        ...
        this.$root.$on("onRegister", (login, userName) => {
          if (login.length < 3 || login.length > 16) {
            this.$root.$emit("onRegisterValidationError", "Login should be from 3 to 16 characters in length");
            return;
          } else if (!(/^[a-z]+$/.test(login))) {
            this.$root.$emit("onRegisterValidationError", "Login should contain only lower latin characters");
            return;
          }

          if (userName.length < 1 || userName.length > 32) {
            this.$root.$emit("onRegisterValidationError", "Name should be from 1 to 32 characters in length");
            return;
          }
